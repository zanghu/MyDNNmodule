...
from multi_AE_0226 import MyMultimodalAutoEncoder, AdjustableMultimodalAutoEncoder
...
def model_evaluate(X_img_train, X_txt_train, y_train, X_img_test, X_txt_test, y_test, layer_num='', prefix='', suffix='', save_path=''):
    ...
    p_r(title=prefix + 'layer' + str(layer_num) + '_test2test_txt2img' + suffix, X_train=X_img_test, label_train=y_test, X_test=X_txt_test, label_test=y_test, pic=True, dist_func='cos', prefix='layer' + str(layer_num) + '_test2test_' + prefix, suffix='_txt2img_cos_' + suffix, save_results=False, save_path=save_path)
    print '1.2测试集图像检索测试及文本:'
    p_r(title=prefix + 'layer' + str(layer_num) + '_test2test_img2txt' + suffix, X_train=X_txt_test, label_train=y_test, X_test=X_img_test, label_test=y_test, pic=True, dist_func='cos', prefix='layer' + str(layer_num) + '_test2test_' + prefix, suffix='_img2txt_cos_' + suffix, save_results=False, save_path=save_path)

    #print '2.1测试集文本检索训练集图像:'
    #p_r(title=prefix + 'layer' + str(layer_num) + '_test2train_txt2img' + suffix, X_train=X_img_train, label_train=y_train, X_test=X_txt_test, label_test=y_test, pic=True, dist_func='cos', prefix='layer' + str(layer_num) + '_test2train_' + prefix, suffix='_txt2img_cos_' + suffix, save_results=False, save_path=save_path)
    #print '2.2测试集图像检索训练集文本'
    #p_r(title=prefix + 'layer' + str(layer_num) + '_test2train_img2txt' + suffix, X_train=X_txt_train, label_train=y_train, X_test=X_img_test, label_test=y_test, pic=True, dist_func='cos', prefix='layer' + str(layer_num) + '_test2train_' + prefix, suffix='_img2txt_cos_' + suffix, save_results=False, save_path=save_path)

    print '3.1训练集文本检索训练集图像:'
    p_r(title=prefix + 'layer' + str(layer_num) + '_train2train_txt2img' + suffix, X_train=X_img_train, label_train=y_train, X_test=X_txt_train, label_test=y_train, pic=True, dist_func='cos', prefix='layer' + str(layer_num) + '_train2train_' + prefix, suffix='_txt2img_cos_' + suffix, save_results=False, save_path=save_path)
    print '3.2训练集图像检索训练集文本'
    p_r(title=prefix + 'layer' + str(layer_num) + '_train2train_img2txt' + suffix, X_train=X_txt_train, label_train=y_train, X_test=X_img_train, label_test=y_train, pic=True, dist_func='cos', prefix='layer' + str(layer_num) + '_train2train_' + prefix, suffix='_img2txt_cos_' + suffix, save_results=False, save_path=save_path)
...
def finish_one_layer(X_train, y_train, X_test, y_test, img_units, txt_units, h_units, epochs, lr=0.1, model_type='FullModal', alpha=0.5, beta=0.5, layer_num='1', prefix='', suffix='', save_path=''):
    ...
    ae_model = AdjustableMultimodalAutoEncoder(model_type=model_type, alpha=alpha, beta=beta, n_vis_img=img_units, n_vis_txt=txt_units, n_hid_img=h_units, n_hid_txt=h_units, dec_f_img=True, dec_f_txt=True, corre_func='cos') #注意2014.03.07新增参数corre_func，用来选择corre_cost的距离函数
