...
01.09: 取消了一部分不必要的print. 将图片集体保存在p_r()函数的save_path下的plots文件夹
"""
import numpy
import time
import os
...
def check_path(path):
    ...
    if path == '':
        ...
        if platform.system() == 'Windows':
            space = '\\'
        path_list = path.split(space) #使用当前系统默认的路径分割符分割path
        #print 'path_list: ', path_list
        cur_path = ''
        for i in path_list:
            cur_path = os.path.join(cur_path, i) #使用当前系统默认路径分割符拼接
            if not os.path.isdir(cur_path):
                #print 'cur_path: ', cur_path
                os.mkdir(cur_path) #如果路径不存在则新建路径
...
def p_r(X_train, label_train, X_test, label_test, dist_func='cos', pic=True, prefix='', suffix='', compute_ma=True, title='', save_results=True, save_path=''):
    """
